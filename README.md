...
- 主窗口启动配置统一保存到便携式 `data/config.json`：实时保存非最大化状态下的逻辑尺寸，并在正常关闭或退出时保存窗口坐标。启动时会恢复有效位置；原显示器不可用、坐标超出当前所有显示器或配置异常时，会在主显示器中央安全启动。旧版 `window-size.json` 会在首次升级启动时自动迁移。
...
- 报告详情默认使用更易阅读的字号；按住 `Ctrl` 滚动鼠标滚轮可在 80%–150% 间缩放正文。缩放直接作用于 Vditor 的实际可编辑内容（包括正文和有序/无序列表），编辑器右上角会显示当前百分比。缩放值保存到 SQLite 偏好设置，重启后自动恢复。
- 单击 Vditor 的标题、普通文本、有序列表或无序列表时都不会显示行级浮动操作条，详情区域不再保留额外的 `editor-accessories` 工具栏。
...
│   │   └── 0007_editor_font_scale.sql             # 报告编辑器字号缩放偏好
│   ├── capabilities/            # Tauri 权限能力配置
│   ├── icons/                   # 各平台应用图标资源
│   ├── Cargo.toml               # Rust 依赖与构建配置
│   └── tauri.conf.json          # Tauri 窗口默认尺寸、构建与打包配置
├── logo.png                     # 应用图标源文件
...
| `src-tauri/migrations/0007_editor_font_scale.sql` | 编辑器字号 | Vditor 正文缩放比例，默认 100% |

开发模式数据库和窗口配置位于 `src-tauri/data/`；发布模式位于应用可执行文件同级的 `data/`。其中 `report-manager.db` 保存业务数据与常规偏好，`config.json` 保存窗口尺寸和位置；两者均被 Git 忽略，不会提交到仓库。
...
7. 在 Vditor 所见即所得编辑区按 `Ctrl+S` 或点击保存图标即可写入 SQLite；工具栏可执行撤销与重做，默认不显示左侧大纲，单击正文行也不会显示行级浮动操作条。按住 `Ctrl` 并滚动鼠标滚轮可缩放正文（80%–150%），右上角显示当前比例，应用会保存并在下次打开时恢复。详情标题栏的按钮从左到右依次为保存、复制报告、删除；复制会立即保存“原标题 - 副本”，删除需确认且不可恢复。复制编辑区选中文本时会保留 Markdown 语法信息，暂不处理图片复制。
8. 在“设置 > 其他”中可配置首次加载与滚动追加报告条数，并可勾选“关闭窗口时最小化到系统托盘”；这些设置修改后立即生效。窗口大小会实时保存，正常关闭或从托盘退出时还会保存当前位置；下次启动会恢复有效的尺寸和位置，失效位置则自动居中到主显示器。
