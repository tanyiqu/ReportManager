...
- 设置页使用单一“菜单管理”入口：可通过左侧拖拽手柄调整自定义菜单并即时保存排序，也可分别重命名、修改 SVG 图标、删除或添加菜单；拖拽基于 Pointer Events 与指针捕获实现，兼容鼠标和触控输入，并以蓝色落点线区分插入到目标菜单之前或之后；指针移出菜单列表时会立即清除落点线；操作按钮使用与语义匹配的 SVG 图标，删除操作以红色垃圾桶标识；首页始终置顶、设置始终置底，且两者不可拖拽或删除。
...
4. 在“设置 > 菜单管理”中按住自定义菜单左侧手柄并拖动，蓝色落点线会标明菜单将插入的位置；指针移出菜单列表时该线会消失，松开后新顺序会立即保存。每行右侧可分别重命名、修改图标或删除。首页和设置不可移动或删除；底部按钮可添加自定义菜单。
5. 设置项会即时写入本地数据库，右上角会显示可排队的“已保存设置”提示；点击任一提示可立即关闭。
6. 点击“保存草稿”目前只会更新前端提示，不会写入数据库。
7. 在“设置 > 其他”中可勾选“关闭窗口时最小化到系统托盘”，修改后立即生效。开启时关闭窗口会隐藏到系统托盘，可点击托盘图标或选择“显示主窗口”恢复；关闭该选项时，点击关闭按钮会直接退出程序。
