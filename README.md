...
- 所有报告菜单共用统一工作区：Header 以同级字号显示“菜单名 ● 报告周期”（周期为灰色）；全文搜索框与仅图标显示的日期筛选按钮位于固定 250px 报告列表顶部，列表和详情区可独立滚动；日期筛选弹层固定从按钮右侧展开，不会被左侧导航遮挡。
- 报告按期日倒序分页加载，首次与滚动追加数量均可在“设置 > 其他”独立配置，默认各为 15 条。
- 报告详情使用所见即所得的 Markdown 编辑区，正文以 Markdown 原文保存到 SQLite；支持浏览器原生撤销、`Ctrl+S` 保存，以及复制时同时写入 Markdown 纯文本和富文本格式。
- 报告详情标题栏的操作按钮依次为保存、复制报告、删除；复制会立即创建并保存一份标题为“原标题 - 副本”的草稿，保留原报告内容和结构化数据。
...
- 表单下拉选择统一使用自定义箭头、焦点环、悬浮和选项配色，与输入框风格保持一致。
...
4. 在“设置 > 菜单管理”中，可用眼睛按钮隐藏或显示报告菜单；隐藏菜单会立即从左侧导航移除，按钮会以蓝色显示激活状态。首页和设置始终可见。非系统菜单还可通过日历按钮设置报告周期，名称同一行会显示“● 周期”。
5. 点击“修改按钮功能顺序”打开独立对话框，水平拖动按钮即可调整每行右侧操作按钮的顺序；松开后立即保存并应用到所有菜单行，不会改变导航菜单的顺序。
6. 设置项会即时写入本地数据库，右上角会显示可排队的“已保存设置”提示；点击任一提示可立即关闭。
7. 在 Markdown 编辑区按 `Ctrl+S` 或点击保存图标即可写入 SQLite；`Ctrl+Z` 可撤销编辑。详情标题栏的按钮从左到右依次为保存、复制报告、删除；复制会立即保存“原标题 - 副本”，删除需确认且不可恢复。复制编辑区选中文本时会保留 Markdown 语法信息，暂不处理图片复制。
