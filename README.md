...
- 所有报告菜单共用统一工作区：Header 以同级字号显示“菜单名 ● 报告周期”（周期为灰色），工具条提供全文搜索和日期筛选；报告列表固定为 250px、详情区自适应宽度，两栏可独立滚动。
...
7. 在 Markdown 编辑区按 `Ctrl+S` 或点击保存图标即可写入 SQLite；`Ctrl+Z` 可撤销编辑。点击详情标题栏的删除图标并确认后，会从 SQLite 永久删除该报告。复制编辑区选中文本时会保留 Markdown 语法信息，暂不处理图片复制。
