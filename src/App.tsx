...
const navigation: { id: Page; label: string; icon: string }[] = [
  { id: "home", label: "首页", icon: "⌂" }, { id: "daily", label: "日报", icon: "▤" },
  { id: "weekly", label: "周报", icon: "▧" }, { id: "meeting", label: "例会记录", icon: "◫" }, { id: "settings", label: "设置", icon: "⚙" },
];
...
export default function App() {
  ...
  const [title, setTitle] = useState(`${today.replace(/星期.*/, "")}日报`);
...
function Settings() { return <section className="panel settings"><h2>偏好设置</h2><label>界面语言<select defaultValue="zh-CN"><option value="zh-CN">简体中文</option></select></label><label>每周起始日<select defaultValue="monday"><option value="monday">周一</option><option value="sunday">周日</option></select></label><label>默认导出目录<input placeholder="尚未设置，导出时选择" /></label><div className="setting-row"><div><h3>本地数据备份</h3><p>导出全部数据以便备份或迁移。内容不会上传到网络。</p></div><button className="secondary">导出全部数据</button></div></section>; }
